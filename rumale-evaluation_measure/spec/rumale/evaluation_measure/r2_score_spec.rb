...
RSpec.describe Rumale::EvaluationMeasure::R2Score do
  ...
  it 'calculates average R^2-score for single regression task', :aggregate_failures do
    r2_score = evaluator.score(ground_truth, estimated)
    expect(r2_score).to be_a(Float)
    expect(r2_score).to be_within(1e-6).of(0.987881)
  end

  it 'calculates average R^2-score for multiple regression task', :aggregate_failures do
    r2_score = evaluator.score(mult_ground_truth, mult_estimated)
    expect(r2_score).to be_a(Float)
    expect(r2_score).to be_within(1e-6).of(0.937039)
  end

  it 'returns zero if the division by zero occurs in calculation process', :aggregate_failures do
