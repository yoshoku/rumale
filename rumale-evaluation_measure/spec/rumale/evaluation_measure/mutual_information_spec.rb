...
RSpec.describe Rumale::EvaluationMeasure::MutualInformation do
  ...
  it 'calculates mutual information of clustering result', :aggregate_failures do
    evaluator = described_class.new
    mi = evaluator.score(ground_truth, predicted)
    expect(mi).to be_a(Float)
    expect(mi).to be_within(1e-4).of(0.462)
    expect(evaluator.score(Numo::Int32[1, 1, 0, 0], Numo::Int32[0, 0, 1, 1])).to be_within(1e-4).of(0.6931)
