...
RSpec.describe Rumale::EvaluationMeasure::MeanAbsoluteError do
  ...
  it 'calculates mean absolute error for single regression task', :aggregate_failures do
    mae = evaluator.score(ground_truth, estimated)
    expect(mae).to be_a(Float)
    expect(mae).to be_within(1e-4).of(0.225)
  end

  it 'calculates mean absolute error for multiple regression task', :aggregate_failures do
