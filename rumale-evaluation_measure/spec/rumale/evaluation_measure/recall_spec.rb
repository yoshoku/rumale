...
RSpec.describe Rumale::EvaluationMeasure::Recall do
  ...
  it 'calculates average recall for binary classification task', :aggregate_failures do
    evaluator = described_class.new(average: 'binary')
    recall = evaluator.score(bin_ground_truth, bin_predicted)
    expect(recall).to be_a(Float)
    expect(recall).to be_within(1e-4).of(0.6)
  end

  it 'calculates macro-average recall for multilabel classification task', :aggregate_failures do
    evaluator = described_class.new(average: 'macro')
    recall = evaluator.score(mult_ground_truth, mult_predicted)
    expect(recall).to be_a(Float)
    expect(recall).to be_within(1e-4).of(0.5625)
  end

  it 'calculates micro-average recall for multilabel classification task', :aggregate_failures do
