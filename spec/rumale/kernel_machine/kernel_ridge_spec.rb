...
RSpec.describe Rumale::KernelMachine::KernelRidge do
  ...
  describe 'single regression problem' do
    let(:y) { x[true, 0] + x[true, 1]**2 }

    it 'learns the model.', aggregate_failures: true do
      ...
      expect(estimator.score(kernel_mat, y)).to be_within(0.01).of(1.0)
    end
    ...
    it 'dumps and restores itself using Marshal module.', aggregate_failures: true do
      ...
      expect(estimator.score(kernel_mat, y)).to eq(copied.score(kernel_mat, y))
    end
  end

  describe 'multiple regression problem' do
  ...
  describe 'validation' do
    it 'raises TypeError when given invalid type value to reg_param.' do
